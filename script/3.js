...
const subSectionAnimate = (selectedSection) => {
  ...
  const imagesLeft = gsap.utils.toArray(
    ".main-assets-section .active.content-section .left-img-wrapper"
  );
  const imagesRight = gsap.utils.toArray(
    ".main-assets-section .active.content-section .right-img-wrapper"
  );

  animateForTab(imagesLeft);
  animateForTab(imagesRight);

  function animateForTab(positionImg) {
    positionImg.forEach((item) => {
